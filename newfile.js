...
;; Clone a bigint
;;
(defun bigint-clone (A)
  (bigint-new (bigint-base A) (copy-sequence (bigint-vec A)) (not (bigint-ge-zero A))))
      ...
      (cond ((= base 65536)
	    ...
	    ((= base (expt 10 ndig))
		 ...
		 (aset vec (- nel (setq i (1+ i)) 1)
		       (string-to-number (substring str 0 ndig)))
  ...
  (save-match-data
    (let ( (A nil) (base 10))
	  ...
	  (let ( len )
	    ...
	    (setq len (length str))
	    (while (> len 4)
	      (setq A (vconcat A (make-vector 1 (string-to-number
						 (substring str (- len 4)) 16))))
	      (setq str (substring str 0 (- len 4)))
	      (setq len (length str)))
	    (setq A (vconcat A (make-vector 1 (string-to-number str 16)))))
	;; Base 10
...
(defun bigint-to-int (X)
  ...
  (string-to-number (bigint-to-string X)))
;;
;; Begin operators
;;
...
(defun bigint-subtract (X Y)
  ...
  (if (not (= (bigint-base X) (bigint-base Y)))
      (error "Both bigints must have the same base"))
  (setq X (bigint-clone X) Y (bigint-clone Y))
  (cond
   ((and (bigint-ge-zero X) (not (bigint-ge-zero Y)))
...
(defun bigint-multiply (X Y)
  ...
  (setq X (bigint-clone X) Y (bigint-clone Y))
  (cond ((bigint-eq-one X) Y) ((bigint-eq-one Y) X)
	((bigint-eq-zero X) X) ((bigint-eq-zero Y) Y) (t
  (let* ( (base (bigint-base X))
	  (positive (or
...
(defun bigint-divide (U V)
  "Divide U by V, both represented as bigints, handling the special
   cases of U < V (both positive) and U = V, otherwise delegating to
   the real workhorse bigint-divide-aux."
  (cond ((and (bigint-ge-zero U) (bigint-ge-zero V)
	      (bigint-less-than U V))
	 (list (bigint-new (bigint-base U) 0) (bigint-clone U)))
	((bigint-equal U V)
	 (list (bigint-new (bigint-base U) 1) (bigint-new (bigint-base U) 0)))
	(t (bigint-divide-aux U V))))

(defun bigint-divide-aux (U V)
...
"
  ...
  (let* ( (base (bigint-base U))
    ...
    (setq Q (make-vector (1+ (max 0 m)) 0))
    ;;
...
;;
;;
(defun bigint-eq-one (X)
  "Is a bigint number X equal to one?"
  (let* ((nonnegative (bigint-ge-zero X))
	 (X (bigint-vec X))
	 (n (length X)) (i 0) (result nil))
    (when (eq (elt X i) 1)
      (setq i (1+ i))
      (setq result nonnegative))
    (while (< i n)
      (if (not (= (elt X i) 0))
	  (setq i (1+ n)))
      (setq i (1+ i)))
    (and result (if (= i n) t nil))))

;;
;; Another couple of helpful comparison operators
;;
(defun bigint-less-than (A B)
  (cdr (cdr (bigint-subtract A B))))

(defun bigint-equal (A B)
  (bigint-eq-zero (bigint-subtract A B)))
